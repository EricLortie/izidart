...
act.art = function( post_id , selector , url){
    ...
    jQuery(function(){
        if( jQuery( selector ).hasClass( 'not-can-vote' ) ){
            ...
            setTimeout( 'act.art_ajax(' + post_id + ')' , 200 );
        }
        console.log("hi!");
    });
    console.log("hi!");
...
act.art_ajax = function( post_id){ //alert(jQuery('#post-'+ post_id).attr('id'));
	alert( jQuery('#post-'+ post_id +' div.set-like').attr('class') );
    jQuery(function(){
        jQuery.post( ajaxurl , {"action" : "art" , "post_id" : post_id} , function( result ) {jQuery('#top_love_'+ post_id ).hide();})
    });
}
...
function add_image_post(){
	...
	var categs = jQuery('#form_post_image').find('#cat').val();

    var categs_string = '';
    if ( categs !== null ) {
        categs.forEach(function(entry) {
            categs_string +='&category__'+entry;
        });
    }
    //console.log(categs_string);
	jQuery.ajax({
		url: ajaxurl,
		data: data+'&action=add_image_post&'+categs_string+window.image_uploader.serialize(),
		type: 'POST',
		cache: false,
...
function add_text_post(){
    ...
    var categs = jQuery('#form_post_text').find('#cat').val();
    ...
    jQuery.ajax({
		url: ajaxurl,
		data: data+'&action=add_text_post'+categs_string,
		type: 'POST',
		cache: false,
		success: function (data) {
			json = eval("(" + data + ")");
...
function add_video_post(){
    ...
    var categs = jQuery('#form_post_video').find('#cat').val();
	...
	jQuery.ajax({
		url: ajaxurl,
		data: data+'&action=add_video_post'+categs_string+window.video_uploader.serialize(),
		type: 'POST',
		cache: false,
		success: function (data) {
			json = eval("(" + data + ")");
...
function add_file_post(){
    ...
    var categs = jQuery('#form_post_file').find('#cat').val();
    var categs_string = '';
    if ( categs !== null ) {
        categs.forEach(function(entry) {
            categs_string +='&category__'+entry;
	...
	jQuery.ajax({
		url: ajaxurl,
		data: data+'&action=add_file_post'+categs_string+window.link_uploader.serialize()+window.link_feat_img_uploader.serialize(),
		type: 'POST',
		cache: false,
		success: function (data) {
			json = eval("(" + data + ")");
...
function add_audio_post(){
    ...
    var categs = jQuery('#form_post_audio').find('#cat').val();
    var categs_string = '';
    if ( categs !== null ) {
        categs.forEach(function(entry) {
            categs_string +='&category__'+entry;
	...
	jQuery.ajax({
		url: ajaxurl,
		data: data+'&action=add_audio_post'+categs_string+window.audio_uploader.serialize()+window.audio_feat_img_uploader.serialize(),
		type: 'POST',
		cache: false,
		success: function (data) {
			json = eval("(" + data + ")");
