...
<?php
                ...
                <header class="entry-header">
                    ...
                    ?>
                                    <div class="love">
                                        ...
                                        <div <?php 	if( like::can_vote( $post -> ID ) ){  echo "onclick=\"javascript:act.like(".$post -> ID.", '#like-".$post -> ID."' , '');\""; } ?> class="set-like voteaction <?php if( !like::can_vote( $post -> ID ) ){ echo "simplemodal-love"; }?> <?php if( like::is_voted( $post -> ID ) ){ echo 'voted'; } ?>" id="voteaction"><em><strong  id="like-<?php echo $post -> ID; ?>"  ><?php  echo count( meta::get_meta( $post ->ID , 'like') ); ?></strong></em></div>
                                    </div>
                    <?php
                                }
                            }else{
                    ...
                    ?>
                            <div class="excerpt">
                                <?php
                                    if( is_user_logged_in () ){
                                        the_excerpt();
                                        echo '<a href="' . get_permalink() . "#more-{$post->ID}\" class=\"more-link\">".__( 'Read more', 'cosmo' ). '</a>';
                                    }else{
                                        $meta = meta::get_meta( $post -> ID  , 'settings' );
                                        if( isset( $meta['safe'] ) ){
                                            if( !meta::logic( $post , 'settings' , 'safe' ) ){
        ...
        static function is_grid( $template ){
            ...
            if ( wp_is_mobile() ) return false; // Using list-view for mobile devices
			...
			if(!isset($_POST['attachments']) || !is_array($_POST['attachments']) || !isset($_POST['featured']) || !is_numeric($_POST['featured']))
			  {
        		...
        		if($is_valid){
                    ...
                    foreach ($_POST as $key_var => $var_value) {
                        if ( strpos($key_var, 'category__') === 0 ) {
